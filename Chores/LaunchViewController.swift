...
class LaunchViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: view.frame.width / 2, height: view.frame.height / 2))
        imageView.translatesAutoresizingMaskIntoConstraints = false
        imageView.image = imageView.resizeImage(image: garbageImage, targetSize: CGSize(width: view.frame.width / 2, height: view.frame.width / 2))
